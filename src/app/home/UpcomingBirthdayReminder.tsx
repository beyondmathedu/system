...
function UpcomingBirthdayReminderFallback({ items }: { items: ReminderItem[] }) {
  return (
      ...
      <div className="mt-2 space-y-1">
        {items.map((item) => (
          <p key={item.id} className="text-sm text-slate-700">
            {`${item.dayLabel} ${item.dateLabel}，${item.personLabel}`}
          </p>
        ))}
      </div>
    </div>
...
export default function UpcomingBirthdayReminder({ items }: { items: ReminderItem[] }) {
  ...
  return (
    <ClientOnlyAfterMount fallback={<UpcomingBirthdayReminderFallback items={items} />}>
      <div className="mt-2 rounded-lg border border-slate-200 bg-white px-3 py-3">
